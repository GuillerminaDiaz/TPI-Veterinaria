...
namespace Veterinaria6.Repository
{
    public class VeterinariaRepository : IVeterinariaRepository
    {
        ...
        public async Task Delete(int id)
        {
            ...
            var detalles = await _context.DetalleAtencions.Where(detalle => detalle.CodAtencion == atencion.CodAtencion).ToListAsync();

            foreach (var detalle in detalles)
            {
                var detalleO = await _context.DetalleAtencions.FindAsync(detalle.CodDetalle);

                _context.DetalleAtencions.Remove(detalleO);
            }
        ...
        public async Task<List<Atencion>> GetByCliente(int userId)
        {
            return await _context.Atencions
            .Where(atencion => atencion.CodMascotaNavigation.CodCliente == userId)
            .Include(atencion => atencion.CodMascotaNavigation)  // Incluye la mascota relacionada
            .Include(atencion => atencion.CodDisponibilidadNavigation)
             .ToListAsync();    
            
            // return new List<Atencion>();
